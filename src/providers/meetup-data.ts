...
export class MeetupData {
  ...
  get(
    ...
    let request = jsonp
      ...
      : this.http
          .get("https://crossorigin.me/" + endpoint, options)
          .map((res) => res.json());
  ...
  getMeetups(topics, lat?, long?): any {
    ...
    if (isDev) {
      return Observable.of(meetupMock);
    }
  ...
  getMeetupGroups(topics, lat?, long?): any {
    ...
    if (isDev) {
      return Observable.of(groupsMock);
    }

    return Observable.forkJoin(requests).flatMap((result: any) =>
      result.map((meetup) => meetup.data)
    );
    // .subscribe( data => {
  ...
  getMeetupHosts(meetupUrl, eventId): any {
    ...
    if (isDev) {
      return Observable.of(hostsMock);
    }
  ...
  getMeetupComments(meetupUrl, eventId): any {
    ...
    if (isDev) {
      return Observable.of(commentsMock);
    }
  ...
  getGroupMembers(group) {
    ...
    if (isDev) {
      return Observable.of(groupMembersMock);
    }
  ...
  getMemberDetails(memberId) {
    ...
    if (isDev) {
      return Observable.of(memberMock);
    }
